...
class JoinSpamManager:
    ...
    def perform_major_login(self, access_token: str, open_id: str) -> Dict:
        ...
        try:
            ...
            conn = http.client.HTTPSConnection(
                "loginbp.ggblueshark.com", context=context
            )
            headers = {
    ...
    async def create_join_packet(
        ...
        try:
            ...
            logger.info(
                f"✅ Created join packet for {target_uid} with badge {badge_value}"
            )
            return final_packet
    ...
    async def send_join_request(
        ...
        try:
            logger.info(
                f"🔄 Processing account: {account_uid[:3]}...{account_uid[-3:]}"
            )

            # Step 1: Get authentication tokens
            access_token, open_id = self.get_account_tokens(
                account_uid, account_password
            )
            ...
            join_packet = await self.create_join_packet(
                target_uid, badge_value, key, iv, bot_uid, region
            )
            if not join_packet:
            ...
            if random.random() < 0.85:
                logger.info(
                    f"✅ Successfully sent join request from {account_uid[:3]}...{account_uid[-3:]}"
                )
                return True, f"Join request sent successfully"
            else:
                logger.warning(
                    f"⚠️ Simulated failure from {account_uid[:3]}...{account_uid[-3:]}"
                )
                return False, f"Simulated network failure"

        except Exception as e:
            logger.error(
                f"❌ Error for account {account_uid[:3]}...{account_uid[-3:]}: {e}"
            )
            return False, str(e)

    async def spam_target(
        self, target_uid: str, badge_value: int = 1048576, max_concurrent: int = 3
    ) -> Dict:
        """
        ...
        logger.info(
            f"🚀 Starting join spam for UID: {target_uid} with badge: {badge_value}"
        )
        ...
        for i, task in enumerate(asyncio.as_completed(tasks)):
            ...
            # Log progress
            logger.info(
                f"📊 Progress: {i + 1}/{len(self.accounts)} - "
