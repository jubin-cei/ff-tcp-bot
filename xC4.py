...
async def xSEndMsgsQ(Msg, id, K, V, region="BD"):
    ...
    fields = {
        ...
        10: "en",
        13: {1: 1, 1: 1},
        14: {11: {1: 3, 2: 7, 3: 170, 4: 999, 5: 1, 6: region, 8: 2, 9: 2}},
    }
...
async def FS(key, iv, region="ind"):
    ...
    try:
        ...
        if len(header_length_final) == 2:
            final_packet_hex = (
                packet_type + "000000" + header_length_final + encrypted_packet
            )
        elif len(header_length_final) == 3:
            final_packet_hex = (
                packet_type + "00000" + header_length_final + encrypted_packet
            )
        elif len(header_length_final) == 4:
            final_packet_hex = (
                packet_type + "0000" + header_length_final + encrypted_packet
            )
        elif len(header_length_final) == 5:
            final_packet_hex = (
                packet_type + "000" + header_length_final + encrypted_packet
            )
        elif len(header_length_final) == 6:
            final_packet_hex = (
                packet_type + "00" + header_length_final + encrypted_packet
            )
        else:
            final_packet_hex = (
                packet_type + "000000" + header_length_final + encrypted_packet
...
async def send_bundle_animation(bundle_id, key, iv, region):
    fields = {1: 88, 2: {1: {1: int(bundle_id)}}}
    if region.lower() == "ind":
        packet = "0514"
    elif region.lower() == "bd":
        packet = "0519"
...
async def GeTSQDaTa(D):
    try:
        ...
        chat_code = (
            data_block.get("8", {}).get("data")
            or data_block.get("17", {}).get("data")
            or data_block.get("18", {}).get("data")
            or ""
        )
        squad_code = (
            data_block.get("8", {}).get("data")
            or data_block.get("31", {}).get("data")
            or ""
        )
